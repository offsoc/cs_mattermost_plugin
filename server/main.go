...
func (p *Plugin) OnActivate() error {
    p.API.LogInfo("CobaltStrike Host Notifier Plugin activated")
    return nil
}
...
func (p *Plugin) sendHostInfoToMattermost(info HostInfo) {
    webhookURL := p.API.GetConfig().PluginSettings.Plugins["cs_mattermost_plugin"].(map[string]interface{})["WebhookURL"].(string)
